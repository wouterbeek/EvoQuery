...
public class Agent2 {
	...
	public Agent2(int memorySize, ArrayList<BindingSet> initialMemory, ArrayList<Integer> params, ArrayList<Integer> termParams) {
		returns = 0;
		score = 0;
		totalScore = 0;
		queryTries = 10;
	...
	public void takeStep(ArrayList<BindingSet> nm) {
		...
		for(int i=0;i<queries.size();i++) {
			try {
			ArrayList<BindingSet> result = Main.rh.queryRepo(queries.get(i));
			returns += result.size();
			int n = 0;
			...
			switch(mp) {
				case 0:
					if(!candidatesS.isEmpty()) {
						...
						if(clause[qp].startsWith("http://"))
							...
							clause[qp] = '"'+clause[qp]+'"';
						String query = start+clause[0]+clause[1]+clause[2]+filter+clause[qp]+end;
						queries.add(query);
					}
					break;
				case 1:
				...
				case 2:
					if(!candidatesO.isEmpty()) {
						if(qp < 2 && !candidatesO.get(0).startsWith("http://")) // we want no literals as subject or predicate 
							continue;
	...
	public void entail() {		
		...
		for(int i=0;i<memory.size();i++) {
			if(memory.get(i).getValue("p").toString().contains("domain")) {   //entailment pattern RDFS2
				for(int j=0;j<memory.size();j++)
					if(memory.get(j).getValue("p").toString().equals(memory.get(i).getValue("x").toString())) {
						...
						if(b.substring(0, 4).equals("http"))
							b = "<"+b+">";						
						String etm = a+"  <http://www.w3.org/1999/02/22-rdf-syntax-ns#type>  "+b;
						if(!entailments.contains(etm)) {
							score++;
							totalScore++;
							entailments.add(etm);
							if(!Main.globalEntailments.contains(etm) && !checkIfEntailmentInDataset(etm)) {
			...
			if(memory.get(i).getValue("p").toString().equals("http://www.w3.org/2000/01/rdf-schema#range")) {  //entailment pattern RDFS3 
				for(int j=0;j<memory.size();j++)
					if(memory.get(j).getValue("p").toString().equals(memory.get(i).getValue("x").toString())) {
						...
						if(b.substring(0, 4).equals("http"))
							b = "<"+b+">";						
						String etm = a+"  <http://www.w3.org/1999/02/22-rdf-syntax-ns#type>  "+b;
						if(!entailments.contains(etm) && !checkIfEntailmentInDataset(etm)) {
							score++;
							totalScore++;
							entailments.add(etm);
			...
			if(memory.get(i).getValue("p").toString().equals("http://www.w3.org/2000/01/rdf-schema#subClassOf")) { //entailment pattern RDFS9
				for(int j=0;j<memory.size();j++)
					if(memory.get(j).getValue("p").toString().equals("<http://www.w3.org/1999/02/22-rdf-syntax-ns#type>") &&
							memory.get(j).getValue("y").equals(memory.get(i).getValue("x"))) {
	...
	public boolean checkIfEntailmentInDataset(String entailment) {
		return (Main.rh.queryRepo("SELECT ?x ?p ?y WHERE { "+entailment+" }").size() > 0);
	}
}
