...
public class Main {
	...
	private void initializeRepository() {
		...
		initialMemory = rh.queryRepo("SELECT ?x ?p ?y WHERE { ?x ?p ?y } LIMIT 300000");
	...
	private void testAgent() {
		...
		for(int i=0;i<initialMemory.size();i++)
			System.out.println(initialMemory.get(i).getValue("x")+" | "+initialMemory.get(i).getValue("p")+" | "+initialMemory.get(i).getValue("y"));
		test.buildQueries();
		test.printQueries();
	}
	...
	private void evolvePopulation(int n) {
		...
		System.out.println(n+". Topscorer: "+population[0].getTotalScore()+"   Top 10: "+top10score+"   Population: "+popscore);	
		
		for(int i=0;i<population.length/10;i++) {
			population[population.length-i-1] = new Agent2(300, initialMemory, population[i].getParams(), population[i].getTermParams());
	...
	public void testCode() {
		ArrayList<BindingSet> ress = rh.queryRepo("SELECT ?x ?p ?y WHERE { <http://dbpedia.org/resource/Connecticut> ?p ?y } LIMIT 30");
		for(int i=0;i<ress.size();i++)
			System.out.println(ress.get(i).getValue("x")+" | "+ress.get(i).getValue("p")+" | "+ress.get(i).getValue("y"));
	}
	...
	private class AgentTotalComparator implements Comparator {
